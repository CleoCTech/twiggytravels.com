                ...
                <x-grid>
                    ...
                    <x-grid-col class="sm:col-span-2">
                        <x-form-group>
                            ...
                            <template #value>
                                <img v-if="show_image == '' && form.image_url != null && form.image_url != 'null'" :src="$page.props.storagePaths[setup.settings.storageName].posts.readPath+form.image_url" class="h-20 w-20">
                                <x-input type="file"  @input="onFileChange($event,'image_url', show_image)" ref="input" class="shadow appearance-none border w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline disabled:opacity-40"/>
...
<script setup>
    ...
    function setData(){
        if (props.cardData != null && props.cardData.uuid != null) {
            ...
            form.image_url = props.cardData.image_url;
